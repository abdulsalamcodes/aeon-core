...
The Aeon core, per [`docs/architecture/ARCHITECTURE.md`](./docs/architecture/ARCHITECTURE.md). A Postgres + Drizzle, RLS-isolated, modular monolith with a transactional outbox and a worker tier. **This is now the only backend** — the legacy Express/Mongo service (`schooler-be`) has been fully ported here and removed. The `schooler-web` frontend runs entirely against this core.
